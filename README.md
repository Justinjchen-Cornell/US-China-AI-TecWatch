...
1. **指标归一化**：对每个指标按其性质做 0-1 归一化（"越高越有利"正向指标线性映射到 [0,1]；"越低越有利"反向指标取补；比率类用对数缩放 `log(1+x)/log(1+x_max)`）。
...
4. **合成**：`Index = 100 * sigmoid( z )`，其中 `z = Σ w_i·(s_i - 0.5)` 经标准化，`sigmoid` 保证输出稳定落在 (0,100)。
5. **不确定性**：对每个指标给一个 `source_reliability ∈ [0,1]`（官方=1.0 / 行业研究=0.8 / 媒体=0.6），指数附带 `confidence = 加权平均可靠度`，并给出 `index_low / index_high`（±1σ 蒙特卡洛扰动，N=2000）。
